    ...
    return(
        <div className="w-full text-white font-mono flex flex-col items-center overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-zinc-800">
            ...
            <div className="h-[90vh] w-full flex flex-col items-center justify-center gap-5">
                <div className="text-2xl anton-font">GAME OF {game.date}</div>
                <div className="p-6">
                <table className="table-fixed border border-[rgba(255,255,255,0.7)]">
                    ...
                    <tr>
                        ...
                        {game["player"].map((player, i) => (
                            <th key={i} className={"w-25 h-12 border border-[rgba(255,255,255,0.7)] text-center "+((game.winner.toLowerCase()==player.toLowerCase())?"bg-[rgba(163,158,20,0.6)]":"bg-[rgba(255,255,255,0.1)]")}>
                    ...
                    {icons.map((icon, rowIndex) => (
                        ...
                        {game[icon[1]].map((value, colIndex) => (
                            <td key={colIndex} className={"w-25 h-12 border border-[rgba(255,255,255,0.7)] text-center "+((rowIndex==icons.length-1)?"bg-[rgba(255,255,255,0.1)]":"")}>{value}</td>
                        ))}
                        </tr>
                    ))}
                    </tbody>
